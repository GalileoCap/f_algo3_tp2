...
Las combinaciones candidatas van a ser los subconjuntos de \(V\) con dos o más elementos, tal que el subgrafo de \(G\) sea conexo, y que todas las \textbf{aristas internas} tengan peso mayor a todas las \textbf{aristas salientes}. Donde una arista interna del subconjunto \(A\) es una arista del grafo donde ambos nodos pertenecen a \(A\), y una arista saliente es una arista del grafo donde sólo uno de los nodos pertenece a \(A\). %TODO: Mejor
